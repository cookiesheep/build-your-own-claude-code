...
**完成项**：
- ✅ 完成后端 12 个 PR 的全面复盘与代码审查
- ✅ 确认后端已从"能跑"进入"可恢复、可回归、可公网演示"阶段
- ✅ 确认三项关键决策：
  - 新增营销首页 `/`，现有首页搬迁到 `/platform`
  - Lab 工作台从固定双栏改为四区域可调整布局（文件树 + 文档 + 编辑器 + 终端）
  - 新增文件目录树（MVP：结构展示 + 目标文件高亮 + 其他文件只读）
- ✅ 降级 GitHub OAuth 为 Bonus（当前匿名身份系统已够用）
- ✅ 确认 API Key 注入方案：容器 ENV + 前端设置面板
- ✅ 更新 `internal/PLATFORM_DESIGN.md`（页面结构、布局、MVP 范围）
- ✅ 创建前端重构规划文档 `internal/FRONTEND_REDESIGN.md`

**进行中**：
- 🔄 前端重构待实施（需要新 Claude Code 会话 + 提示词）

**阻塞项**：
- 无

**验证**：
- `internal/PLATFORM_DESIGN.md` 已更新
- `internal/FRONTEND_REDESIGN.md` 已创建

**下一步**：
- 编写前端重构提示词，交给新 Claude Code 窗口执行
- 后端新增 `GET /api/files/:path` 支持文件目录树
- byocc.cc 部署由另一同学负责
