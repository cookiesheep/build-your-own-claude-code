...
> **⚠️⚠️⚠️ 本文档描述的架构方案已被否决，仅供参考。**
>
> **本文档描述的是"路线 B：独立模块 + 依赖注入"方案（labs/lab-XX/ 独立目录 + DI），团队已决定不采用此方案。**
>
> **实际采用的方案是"路线 A：纯变体文件机制"**：
> - 使用 `-labN` 后缀文件（如 `query-lab3.ts`）+ `build.mjs --lab=N` 编译时替换
> - 学习者在真实 claude-code-diy 源码中填写 skeleton
> - 评测 = 编译验证 + TUI 直接观察 Agent 行为
> - **不使用** Mock LLM、vitest 测试框架、独立模块架构
>
> **详见**：
> - `internal/Lab设计与评测机制-待决问题-2026-04-30.md`（路线 A 已选定）
> - `internal/Lab开发与平台同步手册.md`（变体文件机制操作指南）
> - `internal/prompt/Lab1-3深度设计提示词.md`（Lab 1-3 设计任务）
>
> **本文档中以下内容仍有参考价值**：
> - Lab 总览中的"学习者实现什么"和"反馈"描述（概念层面正确）
> - 教学递进逻辑（Lab 0→3 的渐进式设计思路）
> - 部分测试用例的场景设计可借鉴
>
> **以下内容已过时/不适用**：
> - 独立目录结构（labs/lab-XX/）→ 改为 -labN 后缀变体文件
> - DI 依赖注入架构 → 改为直接在源码中填空
> - vitest + Mock LLM 测试 → 改为编译验证 + TUI 体验
> - types.ts / conversation.ts / llm-client.ts 等独立模块文件 → 改为在 claude-code-diy 源码中挖空对应功能
>
