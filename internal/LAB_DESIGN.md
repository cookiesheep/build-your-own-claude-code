...
> ⚠️ **2026-04-30 标记：本文档部分过时。**
> - 文档描述的"独立模块 + 依赖注入"架构（labs/lab-XX/ 独立目录 + DI）**尚未被团队采用**。
> - 团队目前使用的是**变体文件机制**（`-labN` 后缀文件 + `build.mjs --lab=N` 编译时替换）。
> - 关于 Lab 设计机制的待决问题，见 `internal/Lab设计与评测机制-待决问题-2026-04-30.md`。
> - 本文档的测试用例设计、Mock LLM 方案、教学递进逻辑仍有参考价值，但架构方案需要重新评估。
>
